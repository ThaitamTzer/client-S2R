...
export const columns = [
  ...
  {
    accessor: 'requestedProduct',
    title: 'Sản phẩm cần đổi',
    width: 180,
    render: (record: Exchange) => {
      const product =
        record.role === 'receiver' ? record.requestProduct.requesterProductId : record.receiveProduct.receiverProductId
  ...
  },
  {
    ...
    render: (record: Exchange) => {
      const product =
        record.role === 'receiver' ? record.receiveProduct.receiverProductId : record.requestProduct.requesterProductId
